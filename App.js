...
export default function App() {
  ...
  // Function to insert a single name into the SQLite database
    const insertNameToDb = (name) => {
  ...
  const fetchNamesFromAPI = async () => {
    try {
      const response = await fetch('https://example-data.draftbit.com/people?_limit=10');
      const data = await response.json();
      
      // Upsert each full_name into the SQLite database using existing logic
      const upsertPromises = data.map(person => {
        return new Promise((resolve, reject) => {
          db.transaction(tx => {
            tx.executeSql('INSERT OR REPLACE INTO names (id, name) values (?, ?)', [person.id, person.full_name], // <-- Updated this line
              (txObj, resultSet) => {
                resolve(resultSet.insertId);
              },
              (txObj, error) => {
      ...
      // Fetch names from SQLite to update the state
      db.transaction(tx => {
      ...
      console.error("Error fetching or saving names:", error);
    }
  };
...
////////////////////////////////
  ...
  useEffect(() => {
    ...
    // Fetch names from API and save them to the SQLite database
    fetchNamesFromAPI();
  ...
  return (
    <View style={styles.container}>
      ...
      <Button title="Retrieve Names from API" onPress={fetchNamesFromAPI} />
